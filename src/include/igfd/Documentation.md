...
If you only must display one file dialog at a time, use ImGuiFileDialog's singleton pattern to avoid explicitly
declaring an object:
...
If you must have multiple file dialogs open at once, declare each dialog explicity:
...
if (places_ptr != nullptr) {
	...
	// for the moment the style support only the icon, can be extended if user requiresed in futur
...
Corresponding to your backend (ex : OpenGl) you must define two callbacks :
...
After that you must call the function who is responsible to create / destroy the textures.
...
but you have now a FileSystem interface called IFileSystem who can be overrided with your requiress
...
you juste must set your message for the FileDialogConfig.tooltipMessage
