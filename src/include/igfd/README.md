...
It provides a IFileSystem you can override for your requiress.
...
You must use CMake. For the 3 Os (Win, Linux, MacOs), the CMake usage is exactly the same,
...
Some CMake version requires Build mode define via the directive CMAKE_BUILD_TYPE or via --Config when we launch the build. This is why i put the boths possibilities

By the way you requires before, to make sure, you have requiresed dependencies.

### On Windows :

You must have the opengl library installed

### On Linux :

You requires many lib : (X11, xrandr, xinerama, xcursor, mesa, wayland)
...
you requires many lib : opengl and cocoa framework
